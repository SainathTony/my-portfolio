...
export const useMouseTracking = (config: MouseTrackingConfig = {}): UseMouseTrackingReturn => {
  const {
    trailLength = 6,
    idleTimeout = 3000,
    smoothingFactor = 0.1,
  ...
  const updateMousePosition = useCallback((x: number, y: number, speed: number): void => {
    mouseState.current = {
      ...mouseState.current,
      x,
      y,
  ...
  const updateCursorTrail = useCallback((x: number, y: number, timestamp: number): void => {
    setCursorTrail(prev => {
  ...
  const updateAIAssistantPosition = useCallback((): void => {
    if (!mouseState.current.isIdle) {
      ...
      aiAssistantRef.current.x += 
        (mouseState.current.x - aiAssistantRef.current.x) * assistantFollowSpeed;
      aiAssistantRef.current.y += 
        (mouseState.current.y - aiAssistantRef.current.y) * assistantFollowSpeed;
    } else {
      // Move to corner when idle
      const cornerX = Math.max(0, window.innerWidth - 100);
      const cornerY = Math.max(0, window.innerHeight - 100);
      aiAssistantRef.current.x += (cornerX - aiAssistantRef.current.x) * assistantIdleSpeed;
      aiAssistantRef.current.y += (cornerY - aiAssistantRef.current.y) * assistantIdleSpeed;
    }
  ...
  }, [calculateMouseSpeed, updateMousePosition, updateCursorTrail, resetIdleTimer]);
