...
const PerformanceMonitor: React.FC<PerformanceMonitorProps> = ({
  ...
  useEffect(() => {
    if (performance.memoryInfo) {
      const usagePercent =
        (performance.memoryInfo.usedJSHeapSize /
          performance.memoryInfo.jsHeapSizeLimit) *
        100;
  ...
  if (process.env.NODE_ENV === "development" && showMetrics) {
    return (
      <div
        className={`fixed top-4 left-4 z-50 p-4 bg-black/80 text-white text-xs rounded-lg max-w-xs ${className}`}
      >
        ...
        <div className="space-y-1">
          ...
          <div>
            Reduced Motion: {performance.prefersReducedMotion ? "Yes" : "No"}
          </div>

          {performance.memoryInfo && (
            <div>
              Memory:{" "}
              {Math.round(performance.memoryInfo.usedJSHeapSize / 1024 / 1024)}
              MB
            </div>
          )}
          ...
          <div className="mt-2 text-xs text-gray-400">
            ...
            {performance.supportedFeatures.intersectionObserver ? "✓" : "✗"}
          </div>
        </div>
      </div>
    );
