...
const AIAssistant = React.memo<AIAssistantProps>(
  ({ aiAssistantRef, mouseState }): JSX.Element => {
    const [position, setPosition] = useState<Position>({ x: 0, y: 0 });
    const assistantRef = useRef<HTMLDivElement>(null);
    const animationFrameRef = useRef<number | null>(null);

    // Calculate eye rotation based on mouse position
    const calculateEyeRotation = useCallback(
      ...
      >
        <div className={`relative ${isIdle ? "animate-bounce" : ""}`}>
          ...
          <div
            ...
            style={{
              animation: mouseState?.current?.isIdle
                ? "none"
