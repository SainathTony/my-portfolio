...
export class PerformanceMonitor {
  ...
  static observeElement(element: HTMLElement, callback: () => void, options?: IntersectionObserverInit) {
...
export const getDevicePerformance = (): 'high' | 'medium' | 'low' => {
  ...
  const hasHighPerformanceGPU = /nvidia|amd|intel iris|apple/i.test(renderer.toLowerCase());
  const memoryInfo = (performance as any).memory;
  const hasEnoughMemory = !memoryInfo || memoryInfo.usedJSHeapSize < 100 * 1024 * 1024; // 100MB
  
  if (hasHighPerformanceGPU && hasEnoughMemory && navigator.hardwareConcurrency >= 4) {
    return 'high';
